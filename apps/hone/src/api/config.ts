/** Empty in dev → same-origin + Vite proxy (see vite.config.ts). */
...
/** Prod by default; in dev always same-origin (proxy → prod or localhost). */
...
/** Fallback when /v1/auth/config is unavailable (e.g. local identity without env). */
