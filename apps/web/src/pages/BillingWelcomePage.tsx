  ...
  return (
    <PublicOrAuthedShell>
      <PageContent className="text-center">
        <Eyebrow className={c.muted}>{t('checkout.welcomeEyebrow')}</Eyebrow>
        <h1 className={`mt-2 text-3xl font-semibold tracking-tight ${c.text}`}>
          {isPro ? t('checkout.welcomePro') : t('checkout.welcomePending')}
        </h1>
        {billingQ.data ? (
          <p className={`mx-auto mt-3 max-w-md text-sm ${c.secondary}`}>
